...
The official PDFs are the mechanical authority. `COC7空白卡CY23Final(1).xlsx` is used only as an intake checklist and crosswalk source. Batch 2A and Batch 2B completed the lossless imports supported by the prior Engine. The subsequent Engine-pressure cleanup added the minimal `one-branch` selector and moved `bounty-hunter`, `cowboy`, and `tribe-member` into production. Formal Phase 5B-2 Batch 2 is completed. Batch 3 is now in progress: the Batch 3A through Batch 3E engineering sub-batches completed 65 production definitions and map 76 source rows. Batch 3E imports all 11 Investigator Handbook Criminal subtypes, while the Keeper generic Criminal is withheld as a new Engine pressure. Clerk / Executive remains a separate source semantic ambiguity, and `deprogrammer` remains a separate Engine pressure. Phase 5C has not started.
...
The merged Phase 5B-1 pilot plus Phase 5B-2 Batch 1, completed Batch 2, and Batch 3A through Batch 3E cover:

| Measure | Covered | Total |
| --- | ---: | ---: |
| Canonical families touched | 90 | 91 |
| Fully implemented families | 88 | 91 |
| Production definitions | 116 | 119 expected |
| Official source entries mapped to those definitions | 139 | 142 |

The 139 source-entry rows map to 116 definitions because several canonical definitions retain matching Keeper Rulebook and Investigator Handbook references, while genuine source mechanics variants retain distinct production IDs.

Those definitions touch 90 family identities and fully implement 88. Batch 3A added 17 definitions after PDF re-verification, including two Entertainer source mechanics variants. Batch 3B added 16 canonical definitions after re-verifying all 20 applicable source entries. Batch 3C added 11 definitions across five source-variant families after re-verifying 12 source entries. Batch 3D added 10 definitions mapping 11 source rows: Laborer, Photographer, Pilot, and Sailor are complete, while White-collar Worker has only its confirmed Manager variant in production. Batch 3E added the 11 Handbook Criminal variants; the family remains partial because the Keeper generic Criminal cannot be represented losslessly by the current selector composition. None of these Batch 3 sub-batches changed the Engine.
...
Three confirmed Standard source rows now have `implementation_status=needs-review`: `deprogrammer` for its pre-existing Engine-expression pressure, Keeper Rulebook Criminal for a new choice-pool Engine pressure, and Clerk / Executive under `white-collar-worker` for a source semantic ambiguity. The latter is not an Engine pressure: the printed source says only `Language`, while the same occupation chapter explicitly distinguishes `Own Language` and `Other Language` elsewhere, so choosing either selector would be a guess.

## 8. Engine pressure cases

### Active Engine pressure

#### `criminal` — Keeper Rulebook printed page 40
...
None. `deprogrammer` is unchanged and remains the sole active Engine pressure.
...
Keeper Rulebook printed page 40 Criminal adds `choice-pool-with-repeatable-specialization-branch`: a choose-four pool contains generic Fighting and Firearms children that may each consume multiple distinct specialization refs while still counting as one selected category. Current `one-of`, `all-of`, and `one-branch` composition cannot express that category cardinality losslessly. The source row is `needs-review`; `criminal-keeper-rulebook` is withheld.

The 3 remaining Batch 3 families are:

`criminal`, `deprogrammer`, `white-collar-worker`.

The remaining 3 expected definitions are the withheld Keeper Criminal, Clerk / Executive, and Deprogrammer definitions. Criminal and Deprogrammer retain separate Engine pressures; Clerk / Executive retains its source semantic ambiguity.

## 10. Next production-import numbers

| Measure | Count |
| --- | ---: |
| Current production definitions | 116 |
| Fully implemented families | 88 |
| Partially implemented families with work remaining | 2 (`criminal`, `white-collar-worker`) |
| Wholly unimplemented families | 1 (`deprogrammer`) |
| Families not fully implemented | 3 |
| Expected production definitions remaining | 3 |
...
| Completed Batch 3E fully implemented families / definitions | 0 / 11 (1 partially implemented family touched) |
| Production Batch 3 source entries | 76 |
| Batch 3 families remaining | 3 |
| Batch 3 definitions remaining | 3 across 3 families |
| `needs-review` families | 3 (`criminal`, `deprogrammer`, `white-collar-worker`) |

Phase 5B-2-A intake audit, Batch 1, Batch 2A, Batch 2B, the Batch 2 Engine-pressure cleanup, and Batch 3A through Batch 3E are complete. Formal Batch 2 is completed; Batch 3 is in progress with 3 families / 3 expected definitions remaining. Phase 5B and Phase 5B-2 are still in progress, and Phase 5C has not started.
